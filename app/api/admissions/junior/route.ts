...
export async function POST(req: Request) {
  try {
    ...
    const admission = await JuniorAdmission.create(body);

    // Send confirmation emails (non-blocking)
    sendAdmissionEmails(body, 'Junior').catch(err =>
      console.error('[Junior Admission] Email error:', err)
    );

    return NextResponse.json({ success: true, admission });
  } catch (err: any) {
    console.error('Junior admission error:', err);
