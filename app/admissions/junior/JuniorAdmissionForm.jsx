...
const initialForm = {
  ...
  examDate: "", // chosen common entrance date
};

// ✅ All field components defined OUTSIDE the page component
function Field({
  label,
  name,
  type = "text",
...
export default function JuniorAdmissionForm() {
  ...
  const validate = () => {
    ...
    if (!form.passportPhoto) {
      newErrors.passportPhoto =
        "Please upload a passport photograph of your child.";
    }
    setErrors(newErrors);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${form.surname}_${form.otherNames}_Admission.pdf`);
    } catch (err) {
      console.error(err);
      window.print();
    }
  ...
  const handleSubmit = async () => {
    ...
    if (AMOUNT === 0) {
      const ref = `JNR-FREE-${Date.now()}`;
      setPaymentRef(ref);
      setSubmitting(true);
      try {
        const res = await fetch("/api/admissions/junior", {
          method: "POST",
    ...
    initializePayment({
      onSuccess: async (res) => {
        ...
        setSubmitting(true);
        try {
          const response = await fetch("/api/admissions/junior", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              ...form,
      ...
      <div className="min-h-screen bg-gray-100 py-8 px-4">
        <style>{`
          @media print {
            ...
            .pp-examdate { background: #eef3fc !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          ...
          .pp-title { text-align: center; font-size: 16px; font-weight: bold; letter-spacing: 1.5px; margin: 12px 0 3px; }
          .pp-ref { text-align: center; font-size: 9px; color: #555; border-bottom: 1.5px solid #ccc; padding-bottom: 6px; margin-bottom: 10px; }
          .pp-examdate { text-align: center; margin: 4px 0 10px; padding: 5px 10px; border: 1.5px solid #1a3a6b; font-size: 11px; font-weight: bold; letter-spacing: .4px; }
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="pp-no-print bg-white rounded-2xl shadow-sm border border-green-200 p-6 mb-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Your junior admission form has been received. Print or download a
          ...
          >
            <div id="parent-print-area">
              ...
              {form.examDate && (
                <div className="pp-examdate">
                  📅 COMMON ENTRANCE EXAMINATION DATE:&nbsp;
                  <span style={{ color: "#1a3a6b", fontSize: "12px" }}>
                    {form.examDate}
                  </span>
                </div>
              )}
              ...
              {/* Section A */}
              <div className="pp-section">A. Child&apos;s Biodata</div>
              <div className="pp-fl">
              ...
              <div className="pp-row">
                ...
                <span className="pp-lbl">Position in family</span>
                <span className="pp-val">{form.positionInFamily}</span>
              </div>
              <div className="pp-row">
                <span className="pp-n">6.</span>
                <span className="pp-lbl">Food allergy</span>
                <span className="pp-val">{form.foodAllergy}</span>
                <span className="pp-lbl">Drug allergy</span>
                <span className="pp-val">{form.drugAllergy}</span>
              </div>
              <div className="pp-row">
                <span className="pp-n">7.</span>
                <span className="pp-lbl">Exclusive Breastfeeding?</span>
                <span className="pp-box">{form.exclusiveBreastfeeding}</span>
                <span className="pp-lbl" style={{ marginLeft: 8 }}>
                  No Water?
                </span>
                <span className="pp-box">{form.noWater}</span>
              </div>
              <div className="pp-row">
                <span className="pp-n">8a.</span>
                <span className="pp-lbl">Immunization completed?</span>
                <span className="pp-box">{form.immunizationCompleted}</span>
                <span className="pp-lbl" style={{ marginLeft: 8 }}>
                  Any reaction?
                </span>
                <span className="pp-box">{form.anyReaction}</span>
              </div>
              <div className="pp-row">
                <span className="pp-n">9.</span>
                <span className="pp-lbl">Level of schooling:</span>
                <span className="pp-lbl" style={{ marginLeft: 8 }}>
                  Crèche
                </span>
                <span className="pp-box">
                  {form.levelOfSchooling === "Creche" ? "✓" : ""}
                </span>
                <span className="pp-lbl">Nursery</span>
                <span className="pp-box">
                  {form.levelOfSchooling === "Nursery" ? "✓" : ""}
                </span>
                <span className="pp-lbl">Primary</span>
                <span className="pp-box">
                  {form.levelOfSchooling === "Primary" ? "✓" : ""}
                </span>
              </div>

              {/* Section B */}
              <div className="pp-section">B. Parent / Guardian Biodata</div>
              <div className="pp-fl">
                <span className="pp-n">1.</span>
                <span className="pp-fl-lbl">Surname:</span>
              ...
              <div className="pp-subsec">3. Parents&apos; Signatures</div>
              <div className="pp-row pp-indent">
                <span className="pp-lbl" style={{ minWidth: 48 }}>
                  Father:
                </span>
                <span className="pp-val" style={{ minWidth: 90 }}>
                  {form.fatherSignature}
                </span>
                <span className="pp-lbl">Phone:</span>
              ...
              <div className="pp-fl">
                <span className="pp-n">6.</span>
                <span className="pp-fl-lbl">Parents&apos; Occupation:</span>
                <span className="pp-fl-val">{form.parentsOccupation}</span>
              </div>
              <div className="pp-fl">
                <span className="pp-n">7.</span>
                <span className="pp-fl-lbl">Emergency Contact:</span>
                <span className="pp-fl-val">{form.contactPerson}</span>
              </div>
              <div className="pp-fl">
                <span className="pp-n">8a.</span>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-gray-400 mt-2">
            A non-refundable application fee of ₦15,000 is required. Payment is
            secured by Paystack.
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="flex-1">
              <p className="text-sm text-gray-600 mb-3">
                Upload a clear, recent passport-size photograph of the child.
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 space-y-8">
          ...
          {/* Section A */}
          <div>
            <SectionTitle>A. Child&apos;s Biodata</SectionTitle>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field
                ...
                value={form.positionInFamily}
                onChange={handleChange}
                error={errors.positionInFamily}
              />
              <Field
                label="Food Allergy"
                name="foodAllergy"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              <RadioField
                ...
                onChange={handleChange}
                error={errors.exclusiveBreastfeeding}
              />
              <RadioField
                label="No Water"
                name="noWater"
                ...
                value={form.noWater}
                onChange={handleChange}
                error={errors.noWater}
              />
              <RadioField
                label="Immunization Completed"
                name="immunizationCompleted"
                options={["Yes", "No"]}
                value={form.immunizationCompleted}
                onChange={handleChange}
                error={errors.immunizationCompleted}
              />
              <RadioField
                label="Any Reaction"
                name="anyReaction"
                options={["Yes", "No"]}
                value={form.anyReaction}
                onChange={handleChange}
                error={errors.anyReaction}
              />
            </div>
            ...
            <div className="mt-4">
              <RadioField
                ...
                required
                options={["Creche", "Nursery", "Primary"]}
                value={form.levelOfSchooling}
                onChange={handleChange}
                error={errors.levelOfSchooling}
              />
            </div>
          </div>
          ...
          {/* Section B */}
          <div>
            <SectionTitle>B. Parent / Guardian Biodata</SectionTitle>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field
              ...
              <Field
                ...
                value={form.fatherSignature}
                onChange={handleChange}
                error={errors.fatherSignature}
              />
              <Field
              ...
              <Field
                ...
                placeholder="Full name"
                value={form.motherSignature}
                onChange={handleChange}
                error={errors.motherSignature}
              />
              <Field
              ...
              <Field
                ...
                onChange={handleChange}
                error={errors.parentsOccupation}
              />
              <Field
          ...
          {/* Exam Date Selection */}
          <div>
            <SectionTitle>📅 Common Entrance Examination Date</SectionTitle>
            ...
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 mt-0.5">
                  ...
                  <strong>2 recent passport photographs</strong> of the child on
            ...
            <p className="text-sm text-gray-600 mb-4">
              Select your preferred date for the Common Entrance Examination:
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {["9 May 2026", "11 July 2026", "8 August 2026"].map((date) => (
                <label
                  key={date}
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Common Entrance</p>
                  </div>
                </label>
              ))}
            </div>
            {errors.examDate && (
              <p className="text-xs text-red-500 mt-2">⚠ {errors.examDate}</p>
            )}
          </div>

          {/* Submit */}
          <div className="bg-green-50 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-black">
            <div>
              <p className="text-sm font-semibold text-green-800">
                Form Fee: ₦15000
              </p>
              <p className="text-xs text-green-600 mt-0.5">
                ₦15000 payment required. Click submit to send your application.
              </p>
            </div>
            <button
